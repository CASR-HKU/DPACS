...
__all__ = ['resnet_bottleneck_sparse']


class resnet_bottleneck_sparse_drive:
    def __init__(self, bitfile):
        ...
        self.mask_bits = 64 * 8
        if "parallel" in bitfile:
            self.impl = 'parallel'
            self.P_factor = [64, 16, 16, 64]
        else:
            self.impl = 'serial'
            self.P_factor = [64, 32, 32, 64]
        ...
        self.smask_out_buffer = allocate(shape=(16), dtype=np.int32)
    ...
    def run(self, 
        ...
        c_prune = self.generate_cprune(use_cprune, OC_0, cp_ratio=cp_ratio)
        ...
        self.accel.register_map.cmask_ic = c_prune
        self.accel.register_map.cmask_oc = c_prune
...
class resnet_bottleneck_sparse(resnet_bottleneck_sparse_drive):
    def __init__(self, bitfile):
        super(resnet_bottleneck_sparse, self).__init__(bitfile)

    def resnet50(self, H, W, share_mask, s_ratio=1, c_ratio=1, num_of_test=100):
        ...
        print("Model: ResNet50 with spatial ratio {} and channel ratio {}".format(s_ratio, c_ratio))
    ...
    def resnet101(self, H, W, share_mask, s_ratio=1, c_ratio=1, num_of_test=100):
        self.num_test = num_of_test
        self.batch = 1
        self.log = []
        self.H = H
        ...
        print("Model: ResNet101 with spatial ratio {} and channel ratio {}".format(s_ratio, c_ratio))
        self.first_layer(self.H, self.W) 
        self.H, self.W = self.H // 4, self.W // 4
        self.mask = np.random.choice([0, 1], size=[self.H * self.W], p=[1 - self.nz_ratio, self.nz_ratio])
    ...
    def make_layer(self, planes, blocks, stride, use_cprune=0, enable_pool=0):
        ...
        if stride != 1 or self.inplanes != planes * self.expansion:
            ...
            self.bottleneck(self.inplanes, planes, stride, self.base_width, use_mask=use_mask, use_cprune=use_cprune, enable_pool=enable_pool)
            layer_count += 1
        self.inplanes = planes * self.expansion

        for _ in range(1, blocks):
            use_mask = 1 if (layer_count > 0 and self.share_mask) else 0
            self.bottleneck(self.inplanes, planes, stride=1, base_width=self.base_width, use_mask=use_mask, use_cprune=use_cprune, enable_pool=enable_pool)
            layer_count += 1
    
    def bottleneck(self, inplanes, planes, stride, base_width=64, use_mask=0, use_cprune=0, enable_pool=0):
        width = int(planes * (base_width / 64.0))        

        IC = inplanes
        OC = planes * self.expansion
        if stride == 1:
            self.run(self.H, self.W, IC, width, width, OC, next_c=width, nz_ratio=self.nz_ratio, cp_ratio=self.cp_ratio, stride2=0, res=1, use_mask=use_mask, use_cprune=use_cprune, enable_pool=enable_pool)
        else:
            mask_stride2 = np.zeros([self.batch, self.H, self.W], dtype=np.uint8)
            mask_stride2[:, ::2, ::2] = 1
            self.run(self.H, self.W, IC, IC, IC, OC, next_c=width, mask=mask_stride2, stride2=0, res=0, skip_0=1, skip_3=1, use_mask=1)
    ...
    def last_block(self):
        self.run_2048(stride2=1)
        self.run_2048(stride2=0)
        self.run_2048(stride2=0)
                    
            
            
    def fc(self, num_class=1000, max_c3=256):
        for i in range(math.ceil(num_class / max_c3)):
            self.run(1, 1, 2048, max_c3, max_c3, max_c3, max_c3, use_mask=1, skip_3=1, skip_1=1)
