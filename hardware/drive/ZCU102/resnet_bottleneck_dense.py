...
__all__ = ['resnet_bottleneck_dense']

class resnet_bottleneck_dense_drive:
...
class resnet_bottleneck_dense(resnet_bottleneck_dense_drive):
    def __init__(self, bitfile):
        super(resnet_bottleneck_dense, self).__init__(bitfile)

    def resnet50(self, H, W, num_of_test=100 ):
    ...
    def resnet101(self, H, W, num_of_test=100):
        self.num_test = num_of_test
        self.batch = 1
        self.H = H
        self.W = W
    ...
    def make_layer(self, planes, blocks, stride):
        ...
        if stride != 1 or self.inplanes != planes * self.expansion:
            ...
            self.bottleneck(self.inplanes, planes, stride, self.base_width)
            layer_count += 1
        self.inplanes = planes * self.expansion

        for _ in range(1, blocks):
            self.bottleneck(self.inplanes, planes, stride=1, base_width=self.base_width)
            layer_count += 1
    
    def bottleneck(self, inplanes, planes, stride, base_width=64):
        width = int(planes * (base_width / 64.0))        

        IC = inplanes
        OC = planes * self.expansion
        if stride == 1:
            self.run(self.H , self.W, IC, width, width, OC, stride2=0, res=1)
        else:
            # TODO: software dowsampling of feature
            self.run(self.H // 2, self.W //2, IC, IC, IC, OC, stride2=0, res=0, skip_0=1, skip_3=1)
            self.run(self.H, self.W, IC, width, width, OC, stride2=1, res=1)
    ...
    def run_2048(self, stride2=0):  # The last stage of ResNet exceed the maximum on-chip buffer size, so we split it into multiple kernel calls 
        if stride2 == 0:    
            self.run(self.H, self.W, 2048, 512, 512, 512, skip_3=1, skip_1=1)
            self.run(self.H, self.W, 512, 512, 256, 256, skip_0=1, skip_1=1, stride2=0)
            self.run(self.H, self.W, 512, 512, 256, 256, skip_0=1, skip_1=1, stride2=0)
            self.run(self.H, self.W, 512, 512, 512, 2048, skip_0=1, skip_3=1)
            
        if (stride2 == 1):
            self.run(self.H // 2, self.W // 2, 1024, 512, 512, 512, skip_3=1, skip_1=1)
            self.run(self.H // 2, self.W // 2, 1024, 512, 512, 512, skip_3=1, skip_1=1)
            self.run(self.H // 2, self.W // 2, 1024, 512, 512, 512, skip_3=1, skip_1=1)
            self.run(self.H // 2, self.W // 2, 1024, 512, 512, 512, skip_3=1, skip_1=1)
            
            self.run(self.H, self.W, 1024, 512, 512, 512, skip_3=1, skip_1=1)
            self.run(self.H, self.W, 512, 512, 256, 256, skip_0=1, skip_1=1, stride2=1)
            self.run(self.H, self.W, 512, 512, 256, 256, skip_0=1, skip_1=1, stride2=1)
            self.H, self.W = self.H // 2, self.W// 2
            self.run(self.H, self.W, 512, 512, 512, 2048, skip_0=1, skip_3=1)
    ...
    def fc(self, num_class=1000, max_c3=512):
        for i in range(math.ceil(num_class / max_c3)):
            self.run(1, 1, 2048, max_c3, max_c3, max_c3, skip_3=1, skip_1=1)

    def first_layer(self, H, W):
        if self.impl == "serial":
            self.run(H, W, 64, 64, 64, 64, stride2=1, first_layer=1, skip_0=1, skip_1=1, skip_3=1)
        else:
            self.run(H, W, 16, 16, 64, 64, stride2=1, first_layer=1, skip_0=1, skip_1=1)