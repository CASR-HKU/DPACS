...
int main(){
    ...
    skip_0=0, skip_3=0, skip_1=1, relu=0, residual=1, stride2=0, first_layer=0;
    relu_0 = 1, relu_3 = 1, relu_1 = 0;
    ...
    if (first_layer)
        w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  P4 * 9 * IC_0, "TXT_FILES_w3_dense_layout.txt", w_index);
    if (!skip_0)
    ...
    if (!skip_1)
        w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  OC_3 * OC_1, "TXT_FILES_w3_dense_layout.txt", w_index);
