...
void conv_3x3_double(
	hls::stream<BundleT<9, ap_int<PI * FW> > > &fmap_in,
	...
	typedef BundleT<9, ap_int<PI * FW> > TIN_FPI;
	typedef BundleT<PO, T_P > TOUT_FPO;
	typedef BundleT<9 * PO, ap_int<PI * WW> > TB_W;
...
DO_PRAGMA(HLS ARRAY_PARTITION variable=psum_buffer cyclic factor=PO dim=1)
	...
	T_WPI  w_buffer[9][M_OC][M_IC / PI];
DO_PRAGMA(HLS ARRAY_PARTITION variable=w_buffer cyclic factor=PO dim=2)
...
#pragma HLS PIPELINE
	...
	for(int rep = 0; rep < batch * H_bound * W_bound; rep++){		
		for(int ic = 0; ic < IC / PI; ic++){
			win = fmap_in.read();
			for(int oc = 0; oc < OC / PO; oc++){	
#pragma HLS PIPELINE II=1
				for(int po = 0; po < PO; po++){
					for(int k = 0; k < 9; k++){
						w_vec.data[k + po * 9] = w_buffer[k][oc * PO + po][ic];
					}
				}
...
#pragma HLS ARRAY_PARTITION variable=out.data complete dim=0
	TB_W w_pack;
#pragma HLS ARRAY_PARTITION variable=w_pack.data complete dim=0
...
DO_PRAGMA(HLS ARRAY_PARTITION variable=w_buffer complete dim=1)
	...
	for(T_C k = 0; k < 9; k++){
		for(T_C oc = 0; oc < OC; oc++){
			for(T_C ic = 0; ic < IC / PI; ic++){
#pragma HLS PIPELINE
           		w_buffer[k][oc][ic]= w3[read_count++];
			}
		}
	}
...
#pragma HLS PIPELINE II=1
				for(int po = 0; po < PO; po++){
					for(int k = 0; k < 9; k++){
						w_pack.data[k + po * 9] = w_buffer[k][oc * PO + po][ic];
					}
				}

				for(int po = 0; po < PO; po++){
					T_P psum = psum_buffer[oc * PO + po];
					for(int pi = 0; pi < PI; pi++){
						for(int k = 0; k < 9; k++){
							T_F in = (T_F) win.data[k].range(FW * (pi + 1) -1, FW * pi);
							T_W w_0 = (T_W) (w_pack.data[po * 9 + k].range((pi + 1) * WW - 1, pi * WW));
							psum += in * w_0;
						}
					}
					psum_buffer[oc * PO + po] = psum;
				}
			}
		}
		for(int oc = 0; oc < OC / PO; oc++){	
