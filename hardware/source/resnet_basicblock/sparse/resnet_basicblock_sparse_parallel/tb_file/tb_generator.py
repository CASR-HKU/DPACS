...
class test_bench_generator: 
    def __init__(self, out_dir, PI_FACTOR=[16, 16, 16, 16], CPRUNE_FACTOR=64):
    ...
    #8-bit weights for basic block
    def generate_weight(self): 
        self.w0 = torch.randint(-128, 127, size=(self.OC_0, self.IC_0, 3, 3))
        self.w3 = torch.randint(-128, 127, size=(self.OC_3, self.IC_3, 3, 3))

        self.w0_dense, self.w3_dense  = self.w0.clone(), self.w3.clone()
        ...
        np.savetxt(os.path.join(self.out_dir, self.out_dir + '_w0_dense_layout.txt'), self.transform_w_3x3(self.w0_dense, PI=self.PI_FACTOR[0], PO=self.PI_FACTOR[1]), fmt = '%d')
        ...
        np.savetxt(os.path.join(self.out_dir, self.out_dir + '_w0_sparse_layout.txt'), self.transform_w_3x3(self.w0_dense, PI=self.CPRUNE_FACTOR, PO=self.CPRUNE_FACTOR), fmt = '%d')
    ...
    def dense_conv(self):
        ...
        out0 = F.conv2d(self.input_feature, self.w0_dense, padding=1, stride=1) 
        out0 = self.quantize(out0)
        out0 = F.relu(out0)
        out3 = F.conv2d(out0, self.w3_dense, padding=1, stride=1)
        out3 = self.add_quantize(out3, identity) 
        out3 = F.relu(out3)   
    ...
    def sparse_conv(self):
        ...
        out0 = F.conv2d(self.sparse_input, self.w0, padding=1, stride=1)  #channel pruned weights
        ...
        out3 = self.add_quantize(out3, identity)
        out3 = F.relu(out3)
        out3_sparse = out3 * self.spatial_mask
        ...
        np.savetxt(os.path.join(self.out_dir, self.out_dir + '_sparse_out_3_sparse.txt'), self.transform_feature(out3_sparse), fmt = '%d')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--IC_0', type=int, default=128, help='Number of input channel of first conv 3x3')
    parser.add_argument('--OC_0', type=int, default=128, help='Number of output channel of first conv 3x3')
    parser.add_argument('--OC_3', type=int, default=128, help='Number of output channel of second conv 3x3')
    parser.add_argument('--OC_1', type=int, default=128, help='Number of output channel of potentail conv 1x1')
    ...
    tb_128 = test_bench_generator(output_dir, PI_FACTOR=[16, 16, 16, 16], CPRUNE_FACTOR=32)
