// Elastic linebuffer for second 3x3
template<int PI, int BUFFER_WIDTH>
void conv_3x3_line_buffer_residual_parallel(
...
#pragma HLS UNROLL
	...
	for(ap_uint<20> rep = 0; rep < (MAX_H + 2) * (MAX_H + 2); rep++){
		...
		out_ready = (((new_key.y - old_key.y) == 1) && ((new_key.x - old_key.x) >= 2)) || ((new_key.y - old_key.y) >= 2) || read_end; 
		read_ready = !read_end && (((new_key.y - old_key.y) <= 1) || fifo_empty);
		read_enable = read_ready && !fifo_full;
		out_enable = out_ready;

		// updata bitmap of the buffer
		if(read_enable){
...
#pragma HLS UNROLL
		...
		if(out_enable) {
			...
			out_key.write(old_key);
		}

		if(out_enable && old_key.end == 1) break;
...
#pragma HLS PIPELINE II=1
		...
		bool pop_enable = !fifo_empty && out_enable;
		if (pop_enable) start_pointer = (start_pointer + 1) % FIFO_DEPTH;
	}
	
}

// Elastic linebuffer for first conv 3x3 with stride 2
template<int PI, int BUFFER_WIDTH>
void conv_3x3_line_buffer_stride_parallel(
	hls::stream<BundleT<PI, T_F> > &fmap_in,
	hls::stream<BundleT<9, ap_int<PI * FW> > > &fmap_out,
	hls::stream<T_K > &in_key,
	hls::stream<T_K > &out_key,
...
#pragma HLS UNROLL
		...
		// generate bitmap of output 3x3 window
		out_empty_check = 0;
		for(ap_uint<4> ki = 0; ki < 3; ki++){
#pragma HLS UNROLL
			for(ap_uint<4> kj = 0; kj < 3; kj++){
...
void conv_3x3_line_buffer_wrap(
	...
	if(first_layer){
		...
		conv_3x3_line_buffer_stride_parallel<PI, BUFFER_WIDTH>(
			fmap_in, 
			fmap_out,
			in_key, 
			out_key, 
