...
int main(){
    ...
    static ap_uint<MW> smask_out[65536];
    ap_uint<16> flags = 0;
    int H, W, IC_0, OC_0, OC_3, OC_1, next_c, batch;
    bool skip_0, skip_3, skip_1, relu, residual, stride2, use_mask, enable_pool, use_cprune, first_layer, return_mask, relu0, relu3, relu1;
    int w_index = 0, count = 0, mask_count = 0; 
    bool pass;
    ap_uint<16> cmask_ic = 0, cmask_oc = 0;


    cout<<"-------------start testing dense input--------------"<<endl;
    H=8, W=8, IC_0=128, OC_0=128, OC_3=128, OC_1=128, next_c=128, batch=1;
    skip_0=0, skip_3=0, skip_1=1, residual=1, stride2=0, use_mask=1, enable_pool=0, use_cprune=0, first_layer=0, return_mask=1;
    relu0 = 1, relu3 = 1, relu1 = 0;
    ...
    if (!use_mask)
        w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  IC_0, "TXT_FILESTXT_FILES_w_mask.txt", w_index);
    if (enable_pool)
        w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  OC_1 * OC_1 / CPRUNE_FACTOR, "TXT_FILES_fc.txt", w_index);
    if (first_layer)
	    w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  P4 * IC_0 * 9, "TXT_FILES_w3_dense_layout.txt", w_index);
    if (!skip_0)
        w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  9 * IC_0 * OC_0, "TXT_FILES_w0_dense_layout.txt", w_index);
    if (!skip_3)
    ...
    top(fin, fin, identity, weight_buffer, cmask_out, smask_out, H, W, IC_0, OC_0, OC_3, OC_1, next_c, flags, cmask_ic, cmask_oc, batch);
        
    FILE *f_gt;
    f_gt = fopen("TXT_FILES_dense_out_3.txt", "r");
    cout<<"Comparing with TXT_FILES_dense_out_3.txt"<<endl;
    ...
    H=8, W=8, IC_0=128, OC_0=128, OC_3=128, OC_1=128, next_c=128, batch=1;
    skip_0=0, skip_3=0, skip_1=1,  residual=1, stride2=0, use_mask=1, enable_pool=1, use_cprune=1, first_layer=0, return_mask=1;
    relu0=1, relu3=1, relu1=1;
    ...
    cmask_ic = read_cprune<ap_uint<16> >("TXT_FILES_channel_mask.txt", OC_0 / CPRUNE_FACTOR);
    cmask_oc = cmask_ic;
    cout<<"channel mask is: "<<cmask_ic[0]<<"\t"<<cmask_ic[1]<<"\t"<<cmask_ic[2]<<"\t"<<cmask_ic[3]<<endl;
    ...
    if (enable_pool)
        w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  OC_1 * OC_1 / CPRUNE_FACTOR, "TXT_FILES_fc.txt", w_index);
    if (first_layer)
	    w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  P4 * IC_0 * 9, "TXT_FILES_w3_sparse_layout.txt", w_index);
    if (!skip_0)
        w_index = read_file_static<WW, W_FACTOR>(weight_buffer,  9 * IC_0 * OC_0, "TXT_FILES_w0_sparse_layout.txt", w_index);
    if (!skip_3)
    ...
    sp_gt = fopen("TXT_FILES_sparse_out_3.txt", "r");
    cout<<"Comparing with TXT_FILES_sparse_out_3.txt"<<endl;
    ...
    H=8, W=8, IC_0=128, OC_0=128, OC_3=128, OC_1=128, next_c=128, batch=1;
    skip_0=0, skip_3=0, skip_1=1,  residual=1, stride2=0, use_mask=0, enable_pool=1, use_cprune=1, first_layer=0, return_mask=1;
    relu0=1, relu3=1, relu1=1;
    ...
    spu_gt = fopen("TXT_FILES_sparse_out_3_sparse.txt", "r");
    cout<<"Comparing with TXT_FILES_sparse_out_3_sparse.txt"<<endl;
