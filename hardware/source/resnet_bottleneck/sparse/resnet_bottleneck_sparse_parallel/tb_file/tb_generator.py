...
class test_bench_generator: 
    def __init__(self, out_dir, PI_FACTOR=[64, 16, 16, 64], CPRUNE_FACTOR=64):
    ...
    def generate_weight(self): 
        ...
        np.savetxt(os.path.join(self.out_dir, self.out_dir + '_w3_dense_layout.txt'), self.transform_w_3x3(self.w3_dense, PI=self.PI_FACTOR[1], PO=self.PI_FACTOR[2]), fmt = '%d')
        np.savetxt(os.path.join(self.out_dir, self.out_dir + '_w1_dense_layout.txt'), self.transform_w_1x1(self.w1_dense, PI=self.PI_FACTOR[2], PO=self.PI_FACTOR[3]), fmt = '%d')
...
if __name__ == "__main__":
    ...
    tb_128 = test_bench_generator(output_dir, PI_FACTOR=[64, 16, 16, 64], CPRUNE_FACTOR=64)
