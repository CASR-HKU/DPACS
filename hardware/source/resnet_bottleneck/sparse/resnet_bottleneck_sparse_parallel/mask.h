...
#pragma HLS DATAFLOW
	...
	hls::stream<BundleT<PI, T_F> > fin_s; 
#pragma HLS STREAM variable=fin_s depth=4
	M2S_reduce<PI, W>(fin, fin_s, Height, Width);
	first_layer_key<PI, W>(fin_s, fout, s_key, Height, Width);
}
...
void input_unit(
	...
	else{ // if not obtained spatial mask, load dense input and generate mask
