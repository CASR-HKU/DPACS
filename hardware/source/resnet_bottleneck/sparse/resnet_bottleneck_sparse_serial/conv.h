...
#pragma HLS PIPELINE II=1
						w_buffer[oc * po_factor + po][k * ICPI + ic * pi_factor + pi]= w.read();
					}
				}
			}
		}
...
#pragma HLS PIPELINE II=1
					for(ap_uint<7> po = 0; po < PO; po++){
						w_vec.data[po] = w_buffer[oc * PO + po][k * ICPI + ic];
					}

					// pack 2 MAC on one DSP
