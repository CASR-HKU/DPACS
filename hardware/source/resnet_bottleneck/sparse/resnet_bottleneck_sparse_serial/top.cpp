...
#pragma HLS STREAM variable=f_first depth=2
...
#pragma HLS STREAM variable=f_pool depth=2
    hls::stream<BundleT<PO_P, T_F> > f_pool_in;
#pragma HLS STREAM variable=f_pool depth=2
    hls::stream<BundleT<PO_1, T_F> > fres_s;
#pragma HLS STREAM variable=fres_s depth=3

	hls::stream<T_K > key_first;
#pragma HLS STREAM variable=key_first depth=2
...
#pragma HLS STREAM variable=w_1_s depth=2
    hls::stream<ap_int<PO_P * WW> > fc_s;
...
#pragma HLS STREAM variable=mask_s depth=2
    ...
    bool skip_0      = flags[0];  // 1 - bypass first conv 1x1, 0 - compute first conv 1x1
    bool skip_3      = flags[1];  // 1 - bypass conv 3x3, 0 - compute conv 3x3 
    bool skip_1      = flags[2];  // 1 - bypass last conv 1x1, 0 - compute last conv 1x1
    ...
    bool relu_0      = flags[10]; // 1 - use relu activation on first conv 1x1 layer, 0 - no activation on first conv 1x1 
    ...
    if(use_cprune){
        ...
        ap_uint<4> nz_channel_3 = 0;
        for (ap_uint<4> i = 0; i < 8; i++){
        #pragma HLS UNROLL
            nz_channel_0 += cmask_ic[i];
            nz_channel_3 += cmask_oc[i];
        }
        T_C pruned_OC_0 = nz_channel_0 * CPRUNE_FACTOR;
        T_C pruned_OC_3 = nz_channel_3 * CPRUNE_FACTOR;
    ...
    first_layer_wrap<PI_0, PI_0, 8, 64>(
        f_first,
        f_0_in,
        key_first,
        in_key0,
    ...
    adjust_stream_larger<PO_0, PI_3>
    (
        f_0_out,
        f_3_in,
        out_key0,
    ...
    adjust_stream_smaller<PO_3, PI_1>
    (
        f_3_out,
        f_1_in,
        out_key3,
    ...
    adjust_stream_smaller<PO_1, PO_P >
    (
        f_pool,
        f_pool_in,
        key_pool,
...
#pragma HLS INTERFACE m_axi port=cmask_out bundle=gmem2 depth=65536
