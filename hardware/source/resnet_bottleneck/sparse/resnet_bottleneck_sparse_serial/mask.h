...
template<int PI, typename T_IN>
void merge_key_fmap(
	hls::stream<ap_uint<1> > &s_bool, 
	hls::stream<T_K> &s_key, 
	hls::stream<T_IN> &fin,
	hls::stream<T_IN> &fout,
	int Height, 
	int Width,
	int IC
){
...
#pragma HLS pipeline II = 1
		...
		for(ap_uint<7> i = 0; i < PI / P4; i++){ // don
...
void input_unit(
	...
	else if(use_mask){ // if already obtained spatial mask, load sparse input
		load_mask_wrap<PI, W>(mask, fin, s_key, fout, Height, Width, IC);
	}
	else{ // load dense input and generate spatial mask 
