...
#pragma HLS PIPELINE
	...
	if(first_layer){
		...
		for(ap_uint<32> rep = 0; rep < Height * Width; rep++){
#pragma HLS pipeline II=1
			key = key_in.read();
			T_IN s_read = s_in.read();
			T_OUT pack;
...
#pragma HLS pipeline II=1
		...
		if(return_mask){
			ap_uint<32> offset = Height * Width * OC / PO;
			for(ap_uint<9> i = 0; i < 64 * 64 / MW; i++){
#pragma HLS PIPELINE
				mem[offset + i] = out_mask_buffer[i];
			}
		}
	}
}
...
void Load_Weight_Merge(
	...
	ap_uint<32> total_count = mask_count + w_mask_count + w0_count + w3_count + w1_count + fc_count + w_first_count;
...
#pragma HLS PIPELINE II=1
	...
	for(T_C oc = 0; oc < OC_1 / CPRUNE_FACTOR; oc++){
		for(T_C ic = 0; ic < OC_3 / CPRUNE_FACTOR; ic++){
			bool ic_flag = cmask_oc[ic];
			// assert CPRUNE_FACTOR == PI
			if(ic_flag){
				read_weight_cprune<ap_int<CPRUNE_FACTOR * WW> >(weight, weight_s, read_index);
			}
			read_index += CPRUNE_FACTOR;
...
#pragma HLS PIPELINE 
		ap_int<PI * WW> w_read = weight_s.read();
		for(ap_uint<7> j = 0; j < PI / PO_P; j++){
			ap_int<PO_P * WW> fc = w_read.range((j + 1) * PO_P * WW - 1, j * PO_P * WW);
			fc_s.write(fc);
		}
	}
...
#pragma HLS PIPELINE 
		ap_int<PI * WW> w_read = weight_s.read();
		for(ap_uint<7> j = 0; j < PI / P4; j++){
			ap_int<P4 * WW> w_f = w_read.range((j + 1) * P4 * WW - 1, j * P4 * WW);
			w_first_s.write(w_f);
		}
	}
