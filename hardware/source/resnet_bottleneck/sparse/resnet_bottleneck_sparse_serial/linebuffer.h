...
// This is a small linebuffer that is optimized for first layer with small ic 
template<int PI>
...
#pragma HLS pipeline
	...
	for(ap_int<9> h = -1; h < Height; h++){
		for(ap_int<9> w = -1; w < Width; w++){
			...
			read_flag = (h < Height - 1) && (w < Width - 1); // padding 1					
...
void conv_3x3_line_buffer_residual_stride_serial(
	hls::stream<BundleT<PI, T_F> > &fmap_in,
	...
	bool valid[BUFFER_ROWS][_MAX_H];
#pragma HLS ARRAY_RESHAPE variable=valid complete dim=0
...
#pragma HLS ARRAY_PARTITION variable=win.data complete dim=0
	...
	const int FIFO_DEPTH = _MAX_H * (BUFFER_ROWS);
...
#pragma HLS UNROLL
	...
	for(ap_uint<20> rep = 0; rep < (_MAX_H + 2) * (_MAX_H + 2); rep++){
		...
		if (STRIDE2){
			...
			if(!fifo_empty) old_key = key_fifo[start_pointer]; //peak top index
		}
			...
			for(ap_uint<4> ki = 0; ki < 3; ki++){
				for(ap_uint<4> kj = 0; kj < 3; kj++){
					if(win_valid[ki][kj] == 0){
						...
						T_OFFSET offset = ki * 3 +  kj;
						offset_s.write(offset);
						for(T_C ic = 0; ic < ICPI; ic++){ 
#pragma HLS PIPELINE II=1
							T_FPI output_read = line_buff[(ki - 1 + y) % BUFFER_ROWS][(x + kj - 1) * ICPI + ic];
							TB_FPI output_pack;
							for(ap_uint<7> pi = 0; pi < PI; pi++){
